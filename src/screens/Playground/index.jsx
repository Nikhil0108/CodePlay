...
const Playground = () => {
  ...
  const { title, language: initialLanguage, code: initialCode } = folders[folderId].playgrounds[playgroundId];
  ...
  const postSubmission = async (language_id, source_code, stdin) => {
    ...
    try {
      ...
      console.error('Error posting submission:', error.message || error);
      throw error;
    }
  };
  ...
  const getOutput = async (token) => {
    ...
    try {
      ...
      if (response.data.status_id <= 2) {
        // Wait and try again if the status is not ready
      ...
      console.error('Error getting output:', error.message || error);
      throw error;
    }
  };
  ...
  const runCode = async () => {
    ...
    try {
      ...
      if (res && res.status) {
        ...
        setCurrentOutput('Unexpected response format');
      }
    } catch (error) {
